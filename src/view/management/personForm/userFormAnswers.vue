        ...
        <li v-for="(item,index) in questions" :key=index>
          ...
          <div v-if="item.type == 'radio' ">
            <el-radio-group v-model="answers[index]" class="inputs">
              <el-radio v-for="(i,k) in item.chooses" :key=k :label="i" disabled>{{i}}</el-radio>
            </el-radio-group>
          </div>
          <div v-if="item.type == 'checkbox'">
            <el-checkbox-group v-model="answers[index]" class="inputs">
              <el-checkbox v-for="(i,k) in item.chooses" :key=k :label="i" disabled></el-checkbox>
            </el-checkbox-group>
          </div>
          <textarea name="textAns" v-model="answers[index]" id="textQuest" cols="100" rows="5" v-if="item.chooses == null" disabled></textarea>
        </li>
      </ul>
  ...
  export default {
    data(){
      return {
        ...
        results:[],
        projectId: null,
        formName:this.$route.query.projInfo.projectName
      }
    },
    methods:{
    ...
    mounted () {
        ...
        self.$nextTick(function(){
            personFormJs.selectFormAnswer({userId:self.$route.query.projInfo.userId,formId:self.$route.query.projInfo.formId}).then(res=>{
                for(let i=0;i<self.questions.length;i++){
                    if(self.questions[i].type == 'checkbox'){
                        var jsonStr = res.data.data[i].answer.toString();
                        jsonStr = jsonStr.replace("[", "[\"");
                        jsonStr = jsonStr.replace(/,\s/g, "\",\"");
                        jsonStr = jsonStr.replace("]", "\"]");
                        self.answers[i] = JSON.parse(jsonStr);
                    }else{
                        self.answers[i] = res.data.data[i].answer;
                    }
                }
